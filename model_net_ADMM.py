...
from utils.denoiser_SEnet import Nw_SE
from utils.denoiser_IncpNet import Nw_Incp
from utils.conjugate_gradient import DC
from utils.prox_DCT import DCT_2D_filter, prox_DCTxy
...
class MoDL_Net_ADMM(tf.keras.Model):
    """
    MoDL-ADMM model using CNN denoiser (SKnet) and CG DC-block,
    several modification were made on original MoDL, including learnable sparse regularization and ADMM unrolling
    argmin_x {|Ax-b|^2 + lam*|Nw|^2 + miu*|Dx|_1}
    """

    def __init__(self, mask, **kwargs):
        super(MoDL_Net_ADMM, self).__init__(name='MoDL_Net_ADMM_plus')
        ...
        if self.share_weight in ('nw', 'none'):
            self.lam_list = tf.Variable(tf.constant([0.01] * 10, dtype=tf.float32), trainable=True,
                                        constraint=lambda x: tf.clip_by_value(x, clip_value_min=1e-4,
                                                                              clip_value_max=0.1),
                                        name='lambda_list')
        ...
        # regular weights for DCT sparse term
        self.miu_list = tf.Variable(tf.ones(shape=[10, 8], dtype=tf.float32) * 0.001, trainable=True,
                                    constraint=lambda x: self.clip_by_abs(x, clip_value_min=1e-6, clip_value_max=0.1),
                                    name='miu_list')
        self.DCT = DCT_2D_filter(radius=3, trainable=False)
        # Lagrange penalty weight
        self.rho_list = tf.Variable(tf.constant([0.01] * 11, dtype=tf.float32), trainable=True,
    ...
    def create_Nw(self):
        if self.options['denoiser'] == 'CNN':
            return Nw_CNN(n_l=self.options['nLayer'], n_f=32, kernel_size=3,
        ...
        elif self.options['denoiser'] == 'SE':
            return Nw_SE(n_l=self.options['nLayer'], n_f=32, n_k=3,
                         kernel_initializer=tf.keras.initializers.GlorotNormal())
        elif self.options['denoiser'] == 'Incp':
            return Nw_Incp(n_l=self.options['nLayer'], n_f=32, kernel_initializer=tf.keras.initializers.GlorotNormal())
        else:
            raise ValueError("Unsupported Nw type")

    def clip_by_abs(self, x, clip_value_min=1e-6, clip_value_max=0.1):
        x_abs = tf.abs(x)
        sign = x / (x_abs + 1e-10)
        cliped_x_abs = tf.clip_by_value(x_abs, clip_value_min, clip_value_max)
        return sign * cliped_x_abs
    ...
    def build(self, input_shape):
        ...
        if self.share_weight in ('nw', 'none'):
            ...
            tf.print('miu_list built', self.miu_list.numpy())
            tf.print('rho_list built', self.rho_list.numpy())
            dummy_input = tf.zeros([1] + input_shape[2:] + [2])
            dummy_output = self.Nw(dummy_input, training=False)
    ...
    def call(self, k, csm, training=True, stage=2):
        ...
        def stage2():  # fully functional MoDL_ADMM with stage1 pre_trained Nw
            # initial Auxiliary variable y_0 be Prox_DCT(x_0)
            self.tmp['r_0'] = self.DCT(x_0)
            self.tmp['y_0'] = prox_DCTxy(x_0, self.tmp['r_0'], rho=self.rho_list[0], miu_list=self.miu_list[0, :])
            ...
            for i in range(1, self.niter + 1):
                ...
                if i < self.niter:
                    self.tmp['r_%s' % i] = self.DCT(self.tmp['x_%s' % i])
                    self.tmp['y_%s' % i] = prox_DCTxy(self.tmp['x_%s' % i], self.tmp['r_%s' % i], rho=self.rho_list[i],
                                                      miu_list=self.miu_list[i, :])

                # delete vars to save RAM
                del self.tmp['nw_%s' % i], self.tmp['dw_%s' % i], self.tmp['u_%s' % (i - 2)], \
                    self.tmp['r_%s' % (i - 1)], self.tmp['y_%s' % (i - 1)]
                if i > 1:
